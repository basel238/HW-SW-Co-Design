...
Setup defaults to `python3-dbg` and uses one debug `.venv` for every measurement, including optimization comparisons. It validates `Py_DEBUG=1` and refuses `-O`/`-OO` or an existing release environment before installing dependencies; see **QUICKSTART.md** for preserving an old `.venv` and creating the debug environment. It installs pinned pyperformance 1.14.0, pyperf 2.10.0 and, by default, supplementary py-spy 0.4.2. The former `.venv-guide` is unused and left untouched. System packages are not installed automatically. On the course Ubuntu image, the relevant packages normally include `python3-venv`, `python3-dbg`, `perl`, `linux-tools-common` and the linux-tools package matching `uname -r`. Check availability for your guest kernel; do not change VM permissions just to hide an error.
...
| Each benchmark's timing.json | Unprofiled debug timing data for later analysis |
| framework.json, if available | Actual debug `python -m pyperformance run` result |
...
Normally that is **12 visible files across one run directory and two benchmark directories**, when all graphs are available. Failed artifacts are omitted rather than replaced with placeholders. Intermediate stages no longer create sibling timestamped result directories in this workflow. Console output gives one short status per stage. The duplicate guide-debug stage and its extra graph have been removed.
...
| §2, p3: run/capture/interpret pyperformance | Genuine pinned framework execution using a custom manifest of the frozen sources; separate controlled debug timing | Discuss timing distributions, stability, workload and VM conditions |
...
| Guide p1: python3-dbg | The main pyperformance reference, timing and perf graphs all use the shared debug interpreter | Ensure setup and required capture complete; collect fresh debug baselines |
...
The guide's command is adapted to keep dependency installation, imports and warmup outside the perf-enabled region. The same frozen pyperformance benchmark functions run under debug Python. This is not a byte-for-byte replay of its illustrative 999 Hz whole-harness command; the default uses 499 Hz, software CPU-time events and DWARF unwinding. These choices and the debug interpreter identity are explicit in the evidence.
...
Exit 0 means required debug collection completed. Exit 3 means required evidence is incomplete; open the run README. Exit 2 indicates a configuration/setup error. `--release-only` and the reference command's `--guide` option are no longer accepted. Supplementary failures remain visible in the summary even if required collection succeeds. Existing release evidence stays historical; later optimization comparisons require fresh debug timing for both variants.
